...
rpc.ServiceProxy.prototype.__callMethod = function(methodName, params, successHandler, exceptionHandler, completeHandler){
	...
	if(successHandler && typeof successHandler != 'function')
	    throw Error('The asynchronous onSuccess handler callback function you provided is invalid; the value you provided (' +
			successHandler.toString() + ') is of type "' + typeof(successHandler) + '".');
	if(exceptionHandler && typeof exceptionHandler != 'function')
	    throw Error('The asynchronous onException handler callback function you provided is invalid; the value you provided (' +
			exceptionHandler.toString() + ') is of type "' + typeof(exceptionHandler) + '".');
	if(completeHandler && typeof completeHandler != 'function')
	    throw Error('The asynchronous onComplete handler callback function you provided is invalid; the value you provided (' +
			completeHandler.toString() + ') is of type "' + typeof(completeHandler) + '".');
    }	
	...
	//Calls made with XMLHttpRequest ------------------------------------------------------------
		...
		if(!(params instanceof Object) || params instanceof Date) //JSON-RPC 1.1 allows params to be a hash not just an array
		    throw Error('When making asynchronous calls, the parameters for the method must be passed as an array (or a hash); the value you supplied (' +
				String(params) + ') is of type "' + typeof(params) + '".');
		...
		xhr.onreadystatechange = function(){
		    ...
		    if(xhr.readyState == 4){
			if(xhr.status == 200) { // Check response status  (1.11.13)
			    //XML-RPC
			    if(instance.__protocol == 'XML-RPC'){
				try { // Handle parse XML error  (31.10.13)
				...
				try { // Handle eval JSON error  (31.10.13)
			    ...
			    exceptionHandler('HTTP status: ' + xhr.status + ' (' + xhr.statusText + ')', xhr);
			}
		    }
		};
		...
		if(response.error)
		    throw Error('Unable to call "' + methodName + '". Server responsed with error (code ' +
...
rpc.ServiceProxy.prototype.__doCallback = function(response){
	...
	var message = 'There ' + (uncaughtExceptions.length == 1 ?
				  ...
				  'were ' + uncaughtExceptions.length +
				  ' uncaught exceptions') + ': ';
...
rpc.ServiceProxy.prototype.__upgradeValuesFromJSON = function(obj){
	...
	if(!useHasOwn || obj.hasOwnProperty(key)){
		...
		if(this.__decodeISO8601 &&
		   (matches = obj[key].match(/^(?:(\d\d\d\d)-(\d\d)(?:-(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/))){
		    obj[key] = new Date(0);
	...
	if(valueEl.childNodes.item(i).nodeType == 1){
		...
		while(valueEl){
			...
			if(valueEl.nodeName == 'value')
			    ...
			    throw Error("XML-RPC Parse Error: Illegal element child '" + valueEl.nodeName +
		...
		return arr;
	    default:
		throw Error("XML-RPC Parse Error: Illegal element '" + typeEL.nodeName +
			    "' child of the 'value' element.");
	    }
	}
    }
    return '';
...
rpc.toQueryString = function(params){
	...
	throw Error('You must supply either an array or object type to convert into a query string. You supplied: ' +
		    params.constructor);
	...
	if(useHasOwn && params.hasOwnProperty(key)){
		...
		for(var i = 0; i < params[key].length; i++){
			...
			throw Error('Unable to pass nested arrays nor objects as parameters while in making a cross-site request. The object in question has this constructor: ' +
				    params[key][i].constructor);
		...
		if(params[key] instanceof Date)
		    ...
		    throw Error('Unable to pass objects as parameters while in making a cross-site request. The object in question has this constructor: ' +
